...
//])";
...
#endif
    //qDebug().noquote() << "JSON data is:" << list.toList();
    // Now have error need modify, now I'm modify val = file.readAll();

    //const std::string json = val.toStdString();
    ...
    const std::string TestJson = val.toStdString();
    qDebug().noquote() << "TestJson data is:" << TestJson;
    //qDebug() << "JSON data is:" << json;
